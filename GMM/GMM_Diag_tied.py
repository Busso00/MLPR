...
def mStepDiagonalTied(trainDataV,gammaV,psi=0.01):
    ...
    for g in range(N_COMP):
        ...
        newgau[g]=(newgau[g][0],newgau[g][1],numpy.diagonal(newCg))

    return newgau

def GMMDiagonalTied(trainDataV,gmm,minIncr=10**-6,psi=0.01):
    ...
    while (start or (((llact-llpast)/N_SAMPLES)>minIncr)):
        gmm=newgmm
        gammaV=eStepDiagonalTied(trainDataV,gmm)
        newgmm=mStepDiagonalTied(trainDataV,gammaV,psi=psi)
        if start:
            start=False
...
def LBGDiagonalTied(trainDataV,alpha=0.1,nClusters=2,psi=0.01):#nClusters must be the log2 of the power of 2 nClusters + 1
    N_ATTRS = trainDataV.shape[0]
    ...
    for _ in range(int(numpy.log2(nClusters)+1.0)): 
        if newgmm:
            gmm=newgmm
            gmm=GMMDiagonalTied(trainDataV,gmm,psi=psi)
            gmmV.append(gmm)
...
def allClassLBGDiagonalTied(trainDataV,trainLabelV,alpha=0.1,nComp=2,psi=0.01):
    gmmcV=[]
    for i in  range(N_LABELS):
        gmmV=LBGDiagonalTied(trainDataV[:,trainLabelV==i],nClusters=nComp,alpha=alpha,psi=psi)
        gmmcV.append(gmmV)
...
def inferClassGMMDiagonalTied(testDataV,testLabelV,gmmc,priorProb=PC):
