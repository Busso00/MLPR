...
def GMM(trainDataV,gmm,minIncr=10**-6,psi=0.01):
    ...
    while (start or (((llact-llpast)/N_SAMPLES)>minIncr)):
        gmm=newgmm
        gammaV=eStep(trainDataV,gmm)
        newgmm=mStep(trainDataV,gammaV,psi=psi)
        if start:
            start=False
...
def LBG(trainDataV,alpha=0.1,nClusters=2,psi=0.01):#nClusters must be the log2 of the power of 2 nClusters + 1
    ...
    for _ in range(int(numpy.log2(nClusters)+1.0)): 
        if newgmm:
            gmm=newgmm 
            gmm=GMM(trainDataV,gmm,psi=psi)
            gmmV.append(gmm)
            
        #print("ll")
        #print(GMM_logpdf(trainDataV,gmm)[1].sum()/X.shape[1])
        newgmm=[]
...
def allClassLBG(trainDataV,trainLabelV,alpha=0.1,nComp=2,psi=0.01):
    gmmcV=[]
    for i in  range(N_LABELS):
        gmmV=LBG(trainDataV[:,trainLabelV==i],nClusters=nComp,alpha=alpha,psi=psi)
        gmmcV.append(gmmV)
...
def inferClassGMM(testDataV,testLabelV,gmmc,priorProb=PC):
