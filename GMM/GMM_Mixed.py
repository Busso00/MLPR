...
def mStepDiagTied(trainDataV,gammaV,psi=0.01):
    N_RECORDS = trainDataV.shape[1]
    N_ATTRS= trainDataV.shape[0]
    N_COMP = gammaV.shape[0]#number of components
    ...
    for g in range(N_COMP):
        ...
        newgau[g]=(newgau[g][0],newgau[g][1],numpy.eye(N_ATTRS)*newCg)
...
def GMMDiagTied(trainDataV,gmm,minIncr=10**-6,psi=0.01):
    N_SAMPLES = trainDataV.shape[1]
    start=True
    newgmm=gmm
    llpast=0.0
    ...
    while (start or (((llact-llpast)/N_SAMPLES)>minIncr)):
        gmm=newgmm
        gammaV=eStepTied(trainDataV,gmm)
        newgmm=mStepDiagTied(trainDataV,gammaV,psi=psi)
        if start:
            start=False
            llpast=GMM_logpdf(trainDataV,gmm)[1].sum()
            llact=GMM_logpdf(trainDataV,newgmm)[1].sum()
...
def LBGDiagTied(trainDataV,alpha=0.1,nClusters=2,psi=0.01):#nClusters must be the log2 of the power of 2 nClusters + 1
    N_ATTRS= trainDataV.shape[0]
    mu=vcol(trainDataV.mean(axis=1))
    C=numpy.cov(trainDataV,bias=True).reshape((N_ATTRS,N_ATTRS))#overcome 1 dimensional data
    gmm = [(1.0, mu, C*numpy.eye(N_ATTRS))]
    gmmV = [gmm]
    newgmm=False
    for _ in range(int(numpy.log2(nClusters)+1.0)): 
        if newgmm:
            gmm=newgmm
            gmm=GMMDiagTied(trainDataV,gmm,psi=psi)
            gmmV.append(gmm)
...
def allClassLBGMixed(trainDataV,trainLabelV,alpha=0.1,nComp=2,psi=0.01):
    gmmcV=[]
    
    gmmV=LBGTied(trainDataV[:,trainLabelV==0],nClusters=nComp,alpha=alpha,psi=psi)
    gmmcV.append(gmmV)

    gmmV=LBGDiagTied(trainDataV[:,trainLabelV==1],nClusters=nComp,alpha=alpha,psi=psi)
    gmmcV.append(gmmV)
...
def inferClassGMMMixed(testDataV,testLabelV,gmmc,priorProb=PC):
...
def allClassLBGMixed2(trainDataV,trainLabelV,alpha=0.1,nComp=2,psi=0.01):
    gmmcV=[]
...
def inferClassGMMMixed2(testDataV,testLabelV,gmmc,priorProb=PC):
    N_RECORDS=testDataV.shape[1]
